    ...
    return (
        <section className="rounded-3xl border border-white/10 bg-[#111a2d]/80 p-6">
            <div className="mb-6 flex items-center justify-between gap-4">
                ...
                <button className="shrink-0 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-slate-300">
                    Last 28 days
                </button>
            </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        {learningCalendarData.map((day) => (
                            <div
                                key={day.id}
                                className={`h-11 w-full rounded-md transition hover:ring-2 hover:ring-emerald-400/40 ${intensityStyles[getIntensity(day.value)]
                                    }`}
